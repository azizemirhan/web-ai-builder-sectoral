...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S06-technology-systems/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S06-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S06-technology-systems/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S06 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-technology-heights.json'
...
<title>Energy, Solar &amp; Engineering — S06 Technology Systems · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Technology Systems</h1><p>S06 için beş farklı kompozisyon. Güneş, depolama ve dönüşüm teknolojileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-technology.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-technology.html (5 studies)'
