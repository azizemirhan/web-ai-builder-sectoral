...
$rawPath = Join-Path $workspace 'sectors/education-training/S21-subpage-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S21-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S21-subpage-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S21 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-subpage-hero-heights.json'
...
<title>Education &amp; Training — S21 Subpage Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Subpage Hero</h1><p>S21 için beş farklı kompozisyon. Başvuru ve kayıt alt sayfası için beş modern açılış. Sayfa bağlamı ve tek rehber bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-subpage-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-subpage-hero.html (5 studies)'
