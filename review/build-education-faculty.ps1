...
$rawPath = Join-Path $workspace 'sectors/education-training/S05-instructors-faculty/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S05-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S05-instructors-faculty/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S05 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-faculty-heights.json'
...
<title>Education &amp; Training — S05 Instructors Faculty · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Instructors Faculty</h1><p>S05 için beş farklı kompozisyon. Eğitmenleri tanıtan beş modern düzen. Portre alanları, açılır profil bilgileri ve program kataloğu bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-faculty.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-faculty.html (5 studies)'
