...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S26-person-profile-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S26-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S26-person-profile-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S26 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-profile-detail-heights.json'
...
<title>Dental Clinics — S26 Dentist Profile Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Dentist Profile Detail</h1><p>S26 için beş farklı kompozisyon. Tek hekim profili için beş modern düzen. Kimlik, çalışma odağı, mesleki geçmiş ve iletişim alanları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-profile-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-profile-detail.html (5 studies)'
