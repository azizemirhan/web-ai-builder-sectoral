...
$rawPath = Join-Path $workspace 'sectors/education-training/S08-school-stats/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S08-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S08-school-stats/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S08 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-stats-heights.json'
...
<title>Education &amp; Training — S08 School Stats · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / School Stats</h1><p>S08 için beş farklı kompozisyon. Kurum istatistikleri için beş modern düzen. Sayısal veri alanları, kapsam ve kaynak açıklamaları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-stats.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-stats.html (5 studies)'
