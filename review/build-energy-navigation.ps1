...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S22-breadcrumb-context-navigation/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S22-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S22-breadcrumb-context-navigation/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S22 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-navigation-heights.json'
...
<title>Energy, Solar &amp; Engineering — S22 Breadcrumb / Context Navigation · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Breadcrumb / Context Navigation</h1><p>S22 için beş farklı kompozisyon. Sayfa konumu ve ilgili konular için beş modern gezinme düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-navigation.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-navigation.html (5 studies)'
