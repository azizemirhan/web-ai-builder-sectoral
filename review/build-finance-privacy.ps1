...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S16-security-privacy/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S16-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S16-security-privacy/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S16 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-privacy-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S16 Security Privacy · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Security Privacy</h1><p>S16 için beş farklı kompozisyon. Müşteri deneyimleri ve onaylı yorumlar için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-privacy.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-privacy.html (5 studies)'
