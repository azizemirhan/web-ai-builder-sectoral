...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S06-engagement-process/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S06-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S06-engagement-process/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S06 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-process-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S06 Engagement Process · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Engagement Process</h1><p>S06 için beş farklı kompozisyon. İlk görüşmeden çalışma başlangıcına uzanan süreç için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-process.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-process.html (5 studies)'
