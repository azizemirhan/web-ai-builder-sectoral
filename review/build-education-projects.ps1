...
$rawPath = Join-Path $workspace 'sectors/education-training/S10-student-projects/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S10-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S10-student-projects/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S10 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-projects-heights.json'
...
<title>Education &amp; Training — S10 Student Projects · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Student Projects</h1><p>S10 için beş farklı kompozisyon. Öğrenci projeleri için beş modern galeri. Çalışma görselleri, proje künyeleri ve açılır süreç bilgileri.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-projects.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-projects.html (5 studies)'
