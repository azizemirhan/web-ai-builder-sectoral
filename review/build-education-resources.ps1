...
$rawPath = Join-Path $workspace 'sectors/education-training/S14-resources-downloads/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S14-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S14-resources-downloads/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S14 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-resources-heights.json'
...
<title>Education &amp; Training — S14 Resources Downloads · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Resources Downloads</h1><p>S14 için beş farklı kompozisyon. Kaynaklar ve indirmeler için beş modern düzen. Dosya bilgileri ve açılır kaynak detayları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-resources.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-resources.html (5 studies)'
