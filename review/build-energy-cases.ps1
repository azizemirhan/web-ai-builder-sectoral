...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S14-case-studies/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S14-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S14-case-studies/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S14 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-cases-heights.json'
...
<title>Energy, Solar &amp; Engineering — S14 Case Studies · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Case Studies</h1><p>S14 için beş farklı kompozisyon. Proje hikayeleri ve sonuçları için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-cases.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-cases.html (5 studies)'
