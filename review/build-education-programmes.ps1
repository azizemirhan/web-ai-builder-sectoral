...
$rawPath = Join-Path $workspace 'sectors/education-training/S02-programs-courses/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S02-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S02-programs-courses/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S02 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-programmes-heights.json'
...
<title>Education &amp; Training — S02 Programmes and Courses · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Programmes and Courses</h1><p>S02 için beş farklı kompozisyon. Programlar ve kurslar için beş modern düzen. Karşılaştırılabilir bilgiler ve açılır program ayrıntıları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-programmes.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-programmes.html (5 studies)'
