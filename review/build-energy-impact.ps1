...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S10-sustainability-impact/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S10-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S10-sustainability-impact/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S10 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-impact-heights.json'
...
<title>Energy, Solar &amp; Engineering — S10 Sustainability Impact · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Sustainability Impact</h1><p>S10 için beş farklı kompozisyon. Sürdürülebilirlik yaklaşımı ve etki bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-impact.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-impact.html (5 studies)'
