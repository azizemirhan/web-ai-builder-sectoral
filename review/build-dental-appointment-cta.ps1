...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S20-final-appointment-cta/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S20-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S20-final-appointment-cta/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S20 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-appointment-cta-heights.json'
...
<title>Dental Clinics — S20 Final Appointment CTA · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Final Appointment CTA</h1><p>S20 için beş farklı kompozisyon. Gerçek HTML dosyaları; görsel alanları klinik fotoğrafları için ayrılmıştır.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-appointment-cta.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-appointment-cta.html (5 studies)'
