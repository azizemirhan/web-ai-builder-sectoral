...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S21-subpage-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S21-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S21-subpage-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S21 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-subpage-hero-heights.json'
...
<title>Energy, Solar &amp; Engineering — S21 Subpage Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Subpage Hero</h1><p>S21 için beş farklı kompozisyon. Alt sayfa açılışı için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-subpage-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-subpage-hero.html (5 studies)'
