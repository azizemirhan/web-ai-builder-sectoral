...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S18-locations-offices/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S18-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S18-locations-offices/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S18 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-offices-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S18 Locations Offices · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Locations Offices</h1><p>S18 için beş farklı kompozisyon. Müşteri deneyimleri ve onaylı yorumlar için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-offices.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-offices.html (5 studies)'
