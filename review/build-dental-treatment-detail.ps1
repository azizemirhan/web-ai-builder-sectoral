...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S23-service-offering-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S23-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S23-service-offering-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S23 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-treatment-detail-heights.json'
...
<title>Dental Clinics — S23 Dental Treatment Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Dental Treatment Detail</h1><p>S23 için beş farklı kompozisyon. İlk diş hekimi görüşmesi için kapsam, süreç ve iletişim yolunu anlatan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-treatment-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-treatment-detail.html (5 studies)'
