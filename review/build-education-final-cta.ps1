...
$rawPath = Join-Path $workspace 'sectors/education-training/S20-final-apply-enroll-cta/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S20-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S20-final-apply-enroll-cta/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S20 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-final-cta-heights.json'
...
<title>Education &amp; Training — S20 Final Apply Enroll CTA · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Final Apply Enroll CTA</h1><p>S20 için beş farklı kompozisyon. Başvuruya geçiş için beş modern kapanış. Program seçimi ve başvuru adımlarına yönlendirme.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-final-cta.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-final-cta.html (5 studies)'
