...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S15-technology-integrations/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S15-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S15-technology-integrations/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S15 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-integrations-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S15 Technology Integrations · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Technology Integrations</h1><p>S15 için beş farklı kompozisyon. Firma verileri, tanımlar ve kaynak bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-integrations.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-integrations.html (5 studies)'
