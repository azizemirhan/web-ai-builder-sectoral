...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S24-project-case-study-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S24-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S24-project-case-study-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S24 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-project-detail-heights.json'
...
<title>Energy, Solar &amp; Engineering — S24 Project Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Project Detail</h1><p>S24 için beş farklı kompozisyon. Proje bağlamı, yaklaşım, teslimatlar ve sonuçlar için beş modern detay düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-project-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-project-detail.html (5 studies)'
