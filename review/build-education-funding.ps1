...
$rawPath = Join-Path $workspace 'sectors/education-training/S17-scholarships-funding/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S17-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S17-scholarships-funding/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S17 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-funding-heights.json'
...
<title>Education &amp; Training — S17 Scholarships Funding · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Scholarships Funding</h1><p>S17 için beş farklı kompozisyon. Burs ve destekler için beş modern düzen. Kapsam, uygunluk ve açılır başvuru detayları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-funding.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-funding.html (5 studies)'
