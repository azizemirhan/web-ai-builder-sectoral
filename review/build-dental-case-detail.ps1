...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S24-project-case-study-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S24-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S24-project-case-study-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S24 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-case-detail-heights.json'
...
<title>Dental Clinics — S24 Treatment Case Context Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Treatment Case Context Detail</h1><p>S24 için beş farklı kompozisyon. Anonim vaka bağlamı, kararlar ve takip bilgisi için beş modern düzen. Vaka verileri ve görseller için ayrılmış alanlar içerir.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-case-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-case-detail.html (5 studies)'
