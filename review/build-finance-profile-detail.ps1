...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S26-person-profile-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S26-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S26-person-profile-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S26 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-profile-detail-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S26 Advisor / Expert Profile · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Advisor / Expert Profile</h1><p>S26 için beş farklı kompozisyon. Tek bir kişinin rolü, odağı, bağlı olduğu iş ve iletişim yolu için beş modern profil düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-profile-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-profile-detail.html (5 studies)'
