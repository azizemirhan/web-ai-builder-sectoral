...
$rawPath = Join-Path $workspace 'sectors/education-training/S16-tuition-pricing/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S16-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S16-tuition-pricing/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S16 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-tuition-heights.json'
...
<title>Education &amp; Training — S16 Tuition Pricing · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Tuition Pricing</h1><p>S16 için beş farklı kompozisyon. Eğitim ücretleri için beş modern düzen. Ücret kapsamı, ödeme ve iptal koşulları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-tuition.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-tuition.html (5 studies)'
