...
$rawPath = Join-Path $workspace 'sectors/education-training/S18-locations-online-learning/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S18-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S18-locations-online-learning/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S18 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-locations-heights.json'
...
<title>Education &amp; Training — S18 Locations Online Learning · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Locations Online Learning</h1><p>S18 için beş farklı kompozisyon. Kampüs ve çevrim içi öğrenim için beş modern düzen. Konum, katılım ve erişim detayları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-locations.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-locations.html (5 studies)'
