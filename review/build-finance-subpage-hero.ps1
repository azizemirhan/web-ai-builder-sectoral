...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S21-subpage-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S21-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S21-subpage-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S21 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-subpage-hero-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S21 Subpage Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Subpage Hero</h1><p>S21 için beş farklı kompozisyon. Firma verileri, tanımlar ve kaynak bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-subpage-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-subpage-hero.html (5 studies)'
