...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S03-industries-applications/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S03-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S03-industries-applications/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S03 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-applications-heights.json'
...
<title>Energy, Solar &amp; Engineering — S03 Industries Applications · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Industries Applications</h1><p>S03 için beş farklı kompozisyon. İş yerleri, üretim ve kırsal alanlar için beş modern uygulama düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-applications.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-applications.html (5 studies)'
