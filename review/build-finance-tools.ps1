...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S09-financial-tools/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S09-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S09-financial-tools/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S09 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-tools-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S09 Financial Tools · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Financial Tools</h1><p>S09 için beş farklı kompozisyon. Finansal araçların amacı ve kullanım sınırları için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-tools.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-tools.html (5 studies)'
