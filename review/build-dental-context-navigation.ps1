...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S22-breadcrumb-context-navigation/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S22-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S22-breadcrumb-context-navigation/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S22 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-context-navigation-heights.json'
...
<title>Dental Clinics — S22 Breadcrumb / Context Navigation · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Breadcrumb / Context Navigation</h1><p>S22 için beş farklı kompozisyon. Gerçek bağlantılar ve beş farklı gezinme düzeni. Mobilde tüm yol seviyeleri erişilebilir.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-context-navigation.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-context-navigation.html (5 studies)'
