...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S24-project-case-study-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S24-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S24-project-case-study-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S24 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-case-detail-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S24 Client Engagement / Case Context Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Client Engagement / Case Context Detail</h1><p>S24 için beş farklı kompozisyon. Anonim tek bir etkileşimin bağlamı, kapsamı, aşamaları ve belgelenmiş görsel seti için beş modern detay düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-case-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-case-detail.html (5 studies)'
