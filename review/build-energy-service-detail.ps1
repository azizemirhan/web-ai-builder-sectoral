...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S23-service-offering-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S23-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S23-service-offering-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S23 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-service-detail-heights.json'
...
<title>Energy, Solar &amp; Engineering — S23 Engineering / Energy Solution Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Engineering / Energy Solution Detail</h1><p>S23 için beş farklı kompozisyon. Tek hizmetin kapsamını ve sonraki adımını sunan beş modern detay düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-service-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-service-detail.html (5 studies)'
