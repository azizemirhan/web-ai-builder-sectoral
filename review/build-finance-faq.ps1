...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S17-faq/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S17-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S17-faq/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S17 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-faq-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S17 FAQ · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / FAQ</h1><p>S17 için beş farklı kompozisyon. Firma verileri, tanımlar ve kaynak bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-faq.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-faq.html (5 studies)'
