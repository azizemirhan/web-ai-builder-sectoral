...
$rawPath = Join-Path $workspace 'sectors/education-training/S01-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S01-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S01-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S01 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-hero-heights.json'
...
<title>Education &amp; Training — S01 Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Hero</h1><p>S01 için beş farklı kompozisyon. Eğitim sektörü için beş modern açılış tasarımı. Açılır program bilgisi ve mobil önizleme.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-hero.html (5 studies)'
