...
$rawPath = Join-Path $workspace 'sectors/education-training/S22-breadcrumb-context-navigation/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S22-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S22-breadcrumb-context-navigation/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S22 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-context-navigation-heights.json'
...
<title>Education &amp; Training — S22 Breadcrumb Context Navigation · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Breadcrumb Context Navigation</h1><p>S22 için beş farklı kompozisyon. Sayfa konumu ve ilgili başvuru konuları için beş modern gezinme düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-context-navigation.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-context-navigation.html (5 studies)'
