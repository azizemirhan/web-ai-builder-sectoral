...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S10-client-testimonials/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S10-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S10-client-testimonials/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S10 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-testimonials-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S10 Client Testimonials · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Client Testimonials</h1><p>S10 için beş farklı kompozisyon. Müşteri deneyimleri ve onaylı yorumlar için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-testimonials.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-testimonials.html (5 studies)'
