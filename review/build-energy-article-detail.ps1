...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S25-article-insight-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S25-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S25-article-insight-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S25 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-article-detail-heights.json'
...
<title>Energy, Solar &amp; Engineering — S25 Technical Insight / Article Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Technical Insight / Article Detail</h1><p>S25 için beş farklı kompozisyon. Tek makale için beş modern ve rahat okunabilir düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-article-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-article-detail.html (5 studies)'
