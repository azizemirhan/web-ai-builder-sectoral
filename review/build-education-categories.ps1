...
$rawPath = Join-Path $workspace 'sectors/education-training/S03-course-categories/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S03-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S03-course-categories/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S03 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-categories-heights.json'
...
<title>Education &amp; Training — S03 Course Categories · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Course Categories</h1><p>S03 için beş farklı kompozisyon. Konu alanlarını keşfetmek için beş modern kategori düzeni. Açılır kategori kapsamları ve program kataloğu bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-categories.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-categories.html (5 studies)'
