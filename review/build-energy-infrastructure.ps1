...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S07-infrastructure-equipment/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S07-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S07-infrastructure-equipment/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S07 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-infrastructure-heights.json'
...
<title>Energy, Solar &amp; Engineering — S07 Infrastructure Equipment · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Infrastructure Equipment</h1><p>S07 için beş farklı kompozisyon. Altyapı ve ekipmanlar için görsel ağırlıklı beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-infrastructure.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-infrastructure.html (5 studies)'
