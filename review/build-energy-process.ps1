...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S08-engineering-process/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S08-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S08-engineering-process/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S08 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-process-heights.json'
...
<title>Energy, Solar &amp; Engineering — S08 Engineering Process · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Engineering Process</h1><p>S08 için beş farklı kompozisyon. İlk görüşmeden teslimata kadar süreci sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-process.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-process.html (5 studies)'
