...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S05-capabilities/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S05-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S05-capabilities/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S05 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-capabilities-heights.json'
...
<title>Energy, Solar &amp; Engineering — S05 Capabilities · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Capabilities</h1><p>S05 için beş farklı kompozisyon. Saha değerlendirmesi, sistem koordinasyonu ve teslimat planlaması için beş modern yetkinlik düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-capabilities.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-capabilities.html (5 studies)'
