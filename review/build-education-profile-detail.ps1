...
$rawPath = Join-Path $workspace 'sectors/education-training/S26-person-profile-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S26-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S26-person-profile-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S26 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-profile-detail-heights.json'
...
<title>Education &amp; Training — S26 Instructor Faculty Profile · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Instructor Faculty Profile</h1><p>S26 için beş farklı kompozisyon. Eğitmen kimliği, uzmanlığı, öğretim yaklaşımı ve iletişim için beş modern profil düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-profile-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-profile-detail.html (5 studies)'
