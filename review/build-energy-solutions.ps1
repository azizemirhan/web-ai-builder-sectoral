...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S02-solutions-services/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S02-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S02-solutions-services/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S02 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-solutions-heights.json'
...
<title>Energy, Solar &amp; Engineering — S02 Solutions Services · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Solutions Services</h1><p>S02 için beş farklı kompozisyon. Güneş enerjisi, depolama ve mühendislik desteği için beş modern hizmet düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-solutions.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-solutions.html (5 studies)'
