...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S27-location-branch-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S27-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S27-location-branch-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S27 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-location-detail-heights.json'
...
<title>Energy, Solar &amp; Engineering — S27 Office / Service Location Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Office / Service Location Detail</h1><p>S27 için beş farklı kompozisyon. Adres, yerel iletişim ve ziyaret bilgileri için beş modern konum düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-location-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-location-detail.html (5 studies)'
