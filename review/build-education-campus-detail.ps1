...
$rawPath = Join-Path $workspace 'sectors/education-training/S27-location-branch-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S27-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S27-location-branch-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S27 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-campus-detail-heights.json'
...
<title>Education &amp; Training — S27 Campus Learning Location Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Campus Learning Location Detail</h1><p>S27 için beş farklı kompozisyon. Kampüs, öğrenme ortamı, ulaşım ve ziyaret bilgileri için beş modern detay düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-campus-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-campus-detail.html (5 studies)'
