...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S23-service-offering-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S23-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S23-service-offering-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S23 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-service-detail-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S23 Advisory / Financial Service Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Advisory / Financial Service Detail</h1><p>S23 için beş farklı kompozisyon. Tek bir hizmetin kapsamı, aşamaları, uygunluğu ve başvuru yolu için beş modern detay düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-service-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-service-detail.html (5 studies)'
