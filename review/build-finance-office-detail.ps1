...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S27-location-branch-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S27-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S27-location-branch-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S27 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-office-detail-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S27 Office Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Office Detail</h1><p>S27 için beş farklı kompozisyon. Tek bir ofisin adresi, iletişimi, ziyaret düzeni ve ulaşımı için beş modern ofis düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-office-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-office-detail.html (5 studies)'
