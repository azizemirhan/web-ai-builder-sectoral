...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S19-faq/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S19-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S19-faq/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S19 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-faq-heights.json'
...
<title>Energy, Solar &amp; Engineering — S19 FAQ · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / FAQ</h1><p>S19 için beş farklı kompozisyon. Sık sorulan sorular için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-faq.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-faq.html (5 studies)'
