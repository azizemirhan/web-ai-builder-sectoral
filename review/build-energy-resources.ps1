...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S15-technical-resources/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S15-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S15-technical-resources/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S15 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-resources-heights.json'
...
<title>Energy, Solar &amp; Engineering — S15 Technical Resources · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Technical Resources</h1><p>S15 için beş farklı kompozisyon. Rehber ve kaynakları kapak alanları ve belge bilgileriyle sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-resources.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-resources.html (5 studies)'
