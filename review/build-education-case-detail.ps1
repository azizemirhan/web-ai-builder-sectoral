...
$rawPath = Join-Path $workspace 'sectors/education-training/S24-project-case-study-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S24-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S24-project-case-study-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S24 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-case-detail-heights.json'
...
<title>Education &amp; Training — S24 Cohort Case Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Cohort Case Detail</h1><p>S24 için beş farklı kompozisyon. Öğrenci projesinin bağlamını, gelişimini ve çıktısını sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-case-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-case-detail.html (5 studies)'
