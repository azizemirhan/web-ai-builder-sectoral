...
$rawPath = Join-Path $workspace 'sectors/education-training/S25-article-insight-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S25-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S25-article-insight-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S25 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-article-detail-heights.json'
...
<title>Education &amp; Training — S25 Article Research Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Article Research Detail</h1><p>S25 için beş farklı kompozisyon. Kurs seçimine dair kısa bir rehberi sunan, okuma odaklı beş modern makale düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-article-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-article-detail.html (5 studies)'
