...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S11-capacity-impact-stats/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S11-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S11-capacity-impact-stats/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S11 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-stats-heights.json'
...
<title>Energy, Solar &amp; Engineering — S11 Capacity Impact Stats · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Capacity Impact Stats</h1><p>S11 için beş farklı kompozisyon. Kapasite ve etki göstergelerini kaynaklarıyla sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-stats.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-stats.html (5 studies)'
