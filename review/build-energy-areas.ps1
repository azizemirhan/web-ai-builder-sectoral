...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S16-service-areas/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S16-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S16-service-areas/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S16 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-areas-heights.json'
...
<title>Energy, Solar &amp; Engineering — S16 Service Areas · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Service Areas</h1><p>S16 için beş farklı kompozisyon. Hizmet bölgelerini görsel alanlar ve kapsam bilgileriyle sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-areas.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-areas.html (5 studies)'
