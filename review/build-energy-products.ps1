...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S12-products-systems/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S12-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S12-products-systems/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S12 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-products-heights.json'
...
<title>Energy, Solar &amp; Engineering — S12 Products Systems · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Products Systems</h1><p>S12 için beş farklı kompozisyon. Ürün ve sistemleri görselleri ve detaylarıyla sunan beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-products.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-products.html (5 studies)'
