...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S25-article-insight-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S25-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S25-article-insight-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S25 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-article-detail-heights.json'
...
<title>Dental Clinics — S25 Patient Guide Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Patient Guide Detail</h1><p>S25 için beş farklı kompozisyon. Hasta rehberi için beş modern okuma düzeni. Başlık, makale akışı ve ilgili okuma bağlantıları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-article-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-article-detail.html (5 studies)'
