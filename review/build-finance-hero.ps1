...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S01-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S01-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S01-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S01 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-hero-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S01 Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Hero</h1><p>S01 için beş farklı kompozisyon. Finans, muhasebe ve sigorta sektörü için beş modern açılış düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-hero.html (5 studies)'
