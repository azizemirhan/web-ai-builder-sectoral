...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S13-roi-financing/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S13-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S13-roi-financing/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S13 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-financing-heights.json'
...
<title>Energy, Solar &amp; Engineering — S13 ROI Financing · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / ROI Financing</h1><p>S13 için beş farklı kompozisyon. Yatırım değerlendirmesi ve finansman bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-financing.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-financing.html (5 studies)'
