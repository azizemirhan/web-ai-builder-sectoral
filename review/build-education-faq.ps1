...
$rawPath = Join-Path $workspace 'sectors/education-training/S15-faq/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S15-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S15-faq/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S15 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-faq-heights.json'
...
<title>Education &amp; Training — S15 FAQ · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / FAQ</h1><p>S15 için beş farklı kompozisyon. Sık sorulan sorular için beş modern düzen. İki konu başlığı altında altı açılır yanıt.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-faq.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-faq.html (5 studies)'
