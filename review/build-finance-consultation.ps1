...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S20-consultation-quote-cta/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S20-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S20-consultation-quote-cta/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S20 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-consultation-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S20 Consultation Quote CTA · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Consultation Quote CTA</h1><p>S20 için beş farklı kompozisyon. Firma verileri, tanımlar ve kaynak bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-consultation.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-consultation.html (5 studies)'
