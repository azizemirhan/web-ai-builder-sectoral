...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S26-person-profile-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S26-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S26-person-profile-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S26 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-profile-detail-heights.json'
...
<title>Energy, Solar &amp; Engineering — S26 Engineer / Expert Profile · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Engineer / Expert Profile</h1><p>S26 için beş farklı kompozisyon. Uzman kimliği, biyografi ve iletişim yönlendirmesi için beş modern profil düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-profile-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-profile-detail.html (5 studies)'
