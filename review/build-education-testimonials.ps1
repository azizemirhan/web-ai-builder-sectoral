...
$rawPath = Join-Path $workspace 'sectors/education-training/S09-student-testimonials/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S09-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S09-student-testimonials/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S09 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-testimonials-heights.json'
...
<title>Education &amp; Training — S09 Student Testimonials · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Student Testimonials</h1><p>S09 için beş farklı kompozisyon. Öğrenci deneyimleri için beş modern düzen. Yorum ve portre alanları, açılır bağlam bilgileri.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-testimonials.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-testimonials.html (5 studies)'
