...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S04-projects/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S04-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S04-projects/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S04 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-projects-heights.json'
...
<title>Energy, Solar &amp; Engineering — S04 Projects · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Projects</h1><p>S04 için beş farklı kompozisyon. Proje fotoğrafları ve açılır proje bağlamı için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-projects.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-projects.html (5 studies)'
