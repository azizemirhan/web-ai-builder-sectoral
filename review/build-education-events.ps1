...
$rawPath = Join-Path $workspace 'sectors/education-training/S13-events-webinars/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S13-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S13-events-webinars/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S13 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-events-heights.json'
...
<title>Education &amp; Training — S13 Events Webinars · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Events Webinars</h1><p>S13 için beş farklı kompozisyon. Etkinlik ve webinarlar için beş modern düzen. Oturum bilgileri ve açılır katılım detayları.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-events.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-events.html (5 studies)'
