...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S19-contact/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S19-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S19-contact/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S19 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-contact-heights.json'
...
<title>Dental Clinics — S19 Contact · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Contact</h1><p>S19 için beş farklı kompozisyon. Gerçek HTML dosyaları; görsel alanları klinik fotoğrafları için ayrılmıştır.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-contact.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-contact.html (5 studies)'
