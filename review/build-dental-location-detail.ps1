...
$rawPath = Join-Path $workspace 'sectors/dental-clinics/S27-location-branch-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'DN-S27-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/dental-clinics/S27-location-branch-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five DN-S27 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'dental-location-detail-heights.json'
...
<title>Dental Clinics — S27 Clinic Location Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Dental Clinics / Clinic Location Detail</h1><p>S27 için beş farklı kompozisyon. Tek klinik lokasyonu için beş modern düzen. Adres, iletişim, ulaşım ve erişim bilgileri.</p></header>
...
$destination = Join-Path $PSScriptRoot 'dental-location-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/dental-location-detail.html (5 studies)'
