...
$rawPath = Join-Path $workspace 'sectors/education-training/S12-campus-facilities/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S12-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S12-campus-facilities/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S12 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-campus-heights.json'
...
<title>Education &amp; Training — S12 Campus Facilities · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Campus Facilities</h1><p>S12 için beş farklı kompozisyon. Kampüs ve öğrenim alanları için beş modern düzen. Görsel alanları, konum ve erişim bilgileri.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-campus.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-campus.html (5 studies)'
