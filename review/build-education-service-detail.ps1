...
$rawPath = Join-Path $workspace 'sectors/education-training/S23-service-offering-detail/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S23-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S23-service-offering-detail/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S23 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-service-detail-heights.json'
...
<title>Education &amp; Training — S23 Student Service Detail · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Student Service Detail</h1><p>S23 için beş farklı kompozisyon. Başvuru danışmanlığı için kapsam, hazırlık ve iletişim odaklı beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-service-detail.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-service-detail.html (5 studies)'
