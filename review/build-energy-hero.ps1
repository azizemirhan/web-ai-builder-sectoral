...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S01-hero/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S01-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S01-hero/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S01 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-hero-heights.json'
...
<title>Energy, Solar &amp; Engineering — S01 Hero · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Hero</h1><p>S01 için beş farklı kompozisyon. Güneş enerjisi ve mühendislik için beş modern açılış düzeni.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-hero.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-hero.html (5 studies)'
