...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S20-contact-final-cta/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S20-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S20-contact-final-cta/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S20 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-contact-heights.json'
...
<title>Energy, Solar &amp; Engineering — S20 Contact Final CTA · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Contact Final CTA</h1><p>S20 için beş farklı kompozisyon. İletişim ve son çağrı alanı için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-contact.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-contact.html (5 studies)'
