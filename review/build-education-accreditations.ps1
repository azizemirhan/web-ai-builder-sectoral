...
$rawPath = Join-Path $workspace 'sectors/education-training/S11-accreditations/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S11-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S11-accreditations/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S11 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-accreditations-heights.json'
...
<title>Education &amp; Training — S11 Accreditations · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Accreditations</h1><p>S11 için beş farklı kompozisyon. Akreditasyon kapsamını gösteren beş modern düzen. Kurum, geçerlilik ve açılır doğrulama bilgileri.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-accreditations.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-accreditations.html (5 studies)'
