...
$rawPath = Join-Path $workspace 'sectors/education-training/S07-admissions-enrollment/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S07-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S07-admissions-enrollment/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S07 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-admissions-heights.json'
...
<title>Education &amp; Training — S07 Admissions Enrollment · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Admissions Enrollment</h1><p>S07 için beş farklı kompozisyon. Başvuru ve kayıt sürecini gösteren beş modern düzen. Açılır hazırlık bilgileri ve program kataloğu bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-admissions.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-admissions.html (5 studies)'
