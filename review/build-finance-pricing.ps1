...
$rawPath = Join-Path $workspace 'sectors/finance-accounting-insurance/S08-pricing-packages/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'FIN-S08-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/finance-accounting-insurance/S08-pricing-packages/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five FIN-S08 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'finance-pricing-heights.json'
...
<title>Finance, Accounting &amp; Insurance — S08 Pricing Packages · 5 tasarım</title>
<style>
...
<body>
<header><h1>Finance, Accounting &amp; Insurance / Pricing Packages</h1><p>S08 için beş farklı kompozisyon. Paket kapsamı, ücret bilgileri ve koşullar için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'finance-pricing.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/finance-pricing.html (5 studies)'
