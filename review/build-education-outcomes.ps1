...
$rawPath = Join-Path $workspace 'sectors/education-training/S04-learning-outcomes/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S04-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S04-learning-outcomes/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S04 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-outcomes-heights.json'
...
<title>Education &amp; Training — S04 Learning Outcomes · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Learning Outcomes</h1><p>S04 için beş farklı kompozisyon. Öğrenme kazanımları için beş modern düzen. Değerlendirme açıklaması ve program kataloğu bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-outcomes.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-outcomes.html (5 studies)'
