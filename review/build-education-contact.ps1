...
$rawPath = Join-Path $workspace 'sectors/education-training/S19-contact-admissions/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S19-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S19-contact-admissions/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S19 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-contact-heights.json'
...
<title>Education &amp; Training — S19 Contact Admissions · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Contact Admissions</h1><p>S19 için beş farklı kompozisyon. Başvuru iletişimi için beş modern düzen. İletişim kanalları, çalışma saatleri ve görüşme hazırlığı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-contact.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-contact.html (5 studies)'
