...
$rawPath = Join-Path $workspace 'sectors/education-training/S06-curriculum-learning-path/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'EDU-S06-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/education-training/S06-curriculum-learning-path/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five EDU-S06 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'education-curriculum-heights.json'
...
<title>Education &amp; Training — S06 Curriculum Learning Path · 5 tasarım</title>
<style>
...
<body>
<header><h1>Education &amp; Training / Curriculum Learning Path</h1><p>S06 için beş farklı kompozisyon. Öğrenme aşamalarını gösteren beş modern düzen. Açılır müfredat içerikleri ve program kataloğu bağlantısı.</p></header>
...
$destination = Join-Path $PSScriptRoot 'education-curriculum.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/education-curriculum.html (5 studies)'
