...
$rawPath = Join-Path $workspace 'sectors/energy-solar-engineering/S09-certifications-standards/raw'
$rows = @()
foreach ($file in (Get-ChildItem -LiteralPath $rawPath -Filter 'ENG-S09-*.html' | Sort-Object Name)) {
  ...
  $rows += @{ id = $file.BaseName; title = $title; layout = $layout; url = '../sectors/energy-solar-engineering/S09-certifications-standards/raw/' + $file.Name }
}
if ($rows.Count -ne 5) { throw 'Expected exactly five ENG-S09 studies.' }
$data = ConvertTo-Json -InputObject @($rows) -Depth 5 -Compress
$data = $data.Replace('<', '\u003c')
$measured = '{}'
$heightPath = Join-Path $PSScriptRoot 'energy-credentials-heights.json'
...
<title>Energy, Solar &amp; Engineering — S09 Certifications Standards · 5 tasarım</title>
<style>
...
<body>
<header><h1>Energy, Solar &amp; Engineering / Certifications Standards</h1><p>S09 için beş farklı kompozisyon. Sertifika, standart ve mesleki yeterlilik bilgileri için beş modern düzen.</p></header>
...
$destination = Join-Path $PSScriptRoot 'energy-credentials.html'
[IO.File]::WriteAllText($destination, $output, [Text.UTF8Encoding]::new($false))
Write-Output 'Generated review/energy-credentials.html (5 studies)'
